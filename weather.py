...
def getWeatherData():     
    ...
    headers={
        "User-agent":"Should_i_bring_coat_app"
    }
...
def notify_on_rain(expected_amount_of_rain):
    ...
    hook.send(f"Expecting {expected_amount_of_rain} mm of rain, consider bringing a coat") 

#Searches the data set looking for rain.
for data_series in weather_JSON_Data["properties"]["timeseries"]:
    if "next_6_hours" in data_series["data"]:
        weather_time = datetime.datetime.strptime(data_series["time"],dateformat_string)
        ...
        if delta.seconds/3600 < tracked_hours and delta.days >= 0 and delta.days <1:
            ...
            rain_amount = data_series["data"]["next_6_hours"]["details"]["precipitation_amount"]
